...
def fetch_securites():
    ...
    try:
        ...
        return_portfolio =np.quantile(daily_log[column_to_display].values,loss_quantile)
...
info_label=Label(root, text="Please provide the ticker!",fg='#000000').pack()
entry_field=Entry(root)
entry_field.pack()

info_label2=Label(root, text="Please provide the quantile for which you would like to calculate the metric!", fg='#000000').pack()
number_scale=Scale(root, from_=90, to=100, length=300, tickinterval=0.5, orient=HORIZONTAL, fg='#000000')
number_scale.pack()
calc_button=Button(root, text=text, command=fetch_securites)
result_field=Label(root, fg='#000000')
# Adjust label
